...
class FtpCommands {
  constructor(connection) {
    ...
    this.blacklist = _.get(
      this.connection,
      "server.options.blacklist",
      []
    ).map((cmd) => _.upperCase(cmd));
    this.whitelist = _.get(
      this.connection,
      "server.options.whitelist",
      []
    ).map((cmd) => _.upperCase(cmd));
  }
  ...
  handle(command) {
    ...
    if (
      ...
      !_.includes(this.whitelist, command.directive)
    ) {
      return this.connection.reply(
        502,
    ...
    return Promise.resolve(
      handler({ log, command, previous_command: this.previousCommand })
    ).finally(() => {
